...
export default function LoginForm({ setUser, setAuthState }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (token) {
          try {
            const decodedToken = jwtDecode(token); // Расшифровка токена
        ...
        const redirectPath = location.state?.from || `/profile/${userId}`;
  ...
  return (
    <div className="bg-slate-100 min-h-screen pt-4 md:pt-10">
